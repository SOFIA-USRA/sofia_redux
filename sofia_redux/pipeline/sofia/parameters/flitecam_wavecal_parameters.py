...
class FLITECAMWavecalParameters(FLITECAMSpectroscopyParameters):
    ...
    def identify_lines(self, step_index):
        ...
        if self.config is not None:
            ...
            if 'wavefile' in self.config:
                self.current[step_index].set_value(
                    'wavefile', self.config['wavefile'])
            if 'linefile' in self.config:
                self.current[step_index].set_value(
                    'linefile', self.config['linefile'])

    def fit_lines(self, step_index):
        """
        Modify parameters for the fitting step.

        Sets default spatfile according to `config`.
