...
def validate(valid, validated, status, chop_length, chopping, use_between_scans,
             ...
             instrument_vpa):  # pragma: no cover
    """
    ...
    valid : numpy.ndarray (bool)
        The validity array where `False` marks an invalid frame that should not
        be included in any subsequent operations of shape (n_frames,).  Will be
        updated in-place.  A `False` value on entry overrides any further
        analysis and will remain so.
    validated : numpy.ndarray (bool)
        A boolean array of shape (n_frames,) where `True` indicates that a frame
        has already gone through the validation process and should not be
        re-validated.
    status : numpy.ndarray (int)
        A status array in which values correspond to an observation type.  Those
        that do not correspond to FITS_FLAG_NORMAL_OBSERVING will be flagged as
        invalid.  If `use_between_scans` is `True` and the status is equal to
        FITS_FLAG_BETWEEN_SCANS, the frame will also be marked as invalid.
    ...
    chop_length : numpy.ndarray (float)
        ...
        arcseconds.  If
        |`chop_length` - `chopper_amplitude`| > `transit_tolerance`, then the
        frame will be marked as invalid.  Should be an array of shape
        (n_frames,).
    chopping : bool
    ...
    normal_observing_flag : int
        The integer flag equivalent to the FITS_FLAG_NORMAL_OBSERVING flag.  Any
        `status` value that is not equal to this is flagged as invalid.
    ...
    check_coordinates : numpy.ndarray (bool)
        ...
        `False` if this was a lab observation without any real coordinates.  If
        `False`, the equatorial, object, horizontal, chopper, lst, site, and vpa
        type values will not impact the validity of the frames.  Should be an
    ...
    equatorial_null : numpy.ndarray (bool)
        An array of shape (n_frames,) where `True` indicates that the equatorial
        coordinates are zero valued.  Any such frames will be marked as invalid.
    equatorial_nan : numpy.ndarray (bool)
        An array of shape (n_frames,) where `True` indicates that one or more of
        the equatorial coordinates for a given frame is NaN.  Any such frames
    ...
    horizontal_nan : numpy.ndarray (bool)
        An array of shape (n_frames,) where `True` indicates that the horizontal
        coordinates for a given frame are NaN.  Any such frames will be marked
        as invalid.
    chopper_nan : numpy.ndarray (bool)
        An array of shape (n_frames,) where `True` indicates that one or more of
        the chopper coordinates for a given frame is set to NaN.  If the chopper
        is used, then any such frame will be marked as invalid.
    lst : numpy.ndarray (float)
        The local-sidereal-time values in unix seconds of shape (n_frames,).  If
        any LST value is set to NaN, the associated frame will be marked as
        invalid.
    ...
    telescope_vpa : numpy.ndarray (float)
        The telescope VPA in arbitrary angle units of shape (n_frames,).  If any
        value is set to NaN, the associated frame will be marked as invalid.
    instrument_vpa : numpy.ndarray (float)
        The instrument VPA in arbitrary angle units of shape (n_frames,).  If
        any value is set to NaN, the associated frame will be marked as invalid.
    ...
    """
    n_frames = valid.size
    check_chopper = (chopping and np.isfinite(transit_tolerance) and
                     np.isfinite(chopper_amplitude))
...
def downsample_hwp_angle(hwp_angle, start_indices, valid, window
    ...
    start_indices : numpy.ndarray (int)
        ...
        (low_res_frames,).
    valid : numpy.ndarray (bool)
        A boolean array indicating whether any downsampled value will be valid.
        Should be of shape (low_res_frames,).  Any invalid low-resolution frames
        will be set to NaN.
    window : numpy.ndarray (float)
        The convolution kernel of shape (n_window,).  Since no re-weighting is
        performed, the window should be normalized if applicable.
    ...
    for frame in range(low_frames):
        ...
        for window_index in range(n_window):
            ...
            # The user should make sure this doesn't happen, but just in case...
