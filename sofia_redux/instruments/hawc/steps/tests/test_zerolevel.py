...
from sofia_redux.instruments.hawc.tests.resources \
    import DRPTestCase, scan_smp_data, pol_bgs_data


class TestZeroLevelImage(DRPTestCase):
    def make_data(self, tmpdir, extname='PRIMARY IMAGE'):
        hdul = scan_smp_data()
    ...
    @pytest.mark.parametrize('extname', ['PRIMARY IMAGE', 'STOKES I'])
    def test_siso(self, tmpdir, extname):
        df = self.make_data(tmpdir, extname=extname)
        step = StepZeroLevel()
        out = step(df)

        # check expected output format
        assert isinstance(out, DataFits)
        assert extname in out.imgnames
    ...
    def test_zero_level_auto(self, tmpdir, capsys):
        ...
        assert 'Stokes I zero level: -100' in capt.out
        assert np.nanmin(result1.data) >= 0
        ...
        assert 'Stokes I zero level' not in capt.out
...
class TestZeroLevelPol(DRPTestCase):
    ...
    def test_zero_level_auto(self, tmpdir, capsys):
        ...
        # Add a negative patch to the image
        hdul['STOKES I'].data[5:10, 5:10] = -100.0
        hdul['STOKES Q'].data[5:10, 5:10] = -50.0
        hdul['STOKES U'].data[5:10, 5:10] = -20.0
        zl_rad = [15] * 5
        ...
        assert 'Correcting zero level' in capt.out
        assert 'Stokes I zero level: -100' in capt.out
        assert 'Stokes Q zero level: -50' in capt.out
        assert 'Stokes U zero level: -20' in capt.out
        assert np.nanmin(result1.data) >= 0
        ...
        assert 'No negative zero level' in capt.out
        assert 'Stokes I zero level' not in capt.out
        assert 'Stokes Q zero level' not in capt.out
        assert 'Stokes U zero level' not in capt.out
        assert np.nanmin(result3.data) >= 0
