...
import sofia_redux.toolkit.utilities.fits as u


class TestMergeHeaders(object):
    ...
    def make_headers(self):
        ...
        for i in range(5):
            ...
            header = fits.Header()
            header['NODBEAM'] = beam
            header['AOR_ID'] = f'test_{i}'
            header['ALTI_STA'] = i + 1
            header['ALTI_END'] = i + 10
            header['EXPTIME'] = i * 100 + 1
            header['DTHINDEX'] = i
            hdrl.append(header)
        return hdrl

    def make_config(self):
        kwconf = {'ALTI_STA': 'FIRST', 'ALTI_END': 'LAST',
                  'ASSC_AOR': 'CONCATENATE', 'ASSC_MSN': 'CONCATENATE',
                  'ASSC_OBS': 'CONCATENATE', 'DETITIME': 'MEAN',
                  'DTHINDEX': 'DEFAULT', 'EXPTIME ': 'SUM',
                  'FBC-STAT': 'LAST', 'FOCUS_ST': 'FIRST', 'FOCUS_EN': 'LAST',
                  'IMAGEID ': 'DEFAULT', 'LAT_END ': 'LAST',
                  'LON_END ': 'LAST', 'MISSN-ID': 'FIRST',
                  'NEXP    ': 'SUM', 'PLANID  ': 'FIRST',
                  'SIBS_X  ': 'DEFAULT', 'SIBS_Y  ': 'DEFAULT',
                  'TOTINT  ': 'SUM', 'TRACERR ': 'OR',
                  'TSC-STAT': 'LAST', 'UTCSTART': 'FIRST',
                  'UTCEND  ': 'LAST', 'WVZ_STA ': 'FIRST',
                  'WVZ_END ': 'LAST', 'ZA_START': 'FIRST',
                  'ZA_END  ': 'LAST'}
        return kwconf
    ...
    def test_success(self):
        ...
        merged = u.merge_headers(hdrl, kwconf)
    ...
    def test_ref_header(self):
        ...
        merged = u.merge_headers(hdrl, kwconf, reference_header=ref)
    ...
    def test_operations(self, tmpdir, mocker, capsys):
        ...
        # merge
        merge = u.merge_headers(hdrl, str(conf))
